...
static int cmd_manet_rildinit(char *args[], int num, struct connman_option *options)
{
	DBusMessage * msg;

	msg = dbus_message_new_method_call("org.ofono", "/manet",
			"org.ofono.Manet", "RildInit");
	if (!msg)
		return -ENOMEM;
...
static int cmee = 1;
static void append_cmee(DBusMessageIter *iter, void *data)
{
	fprintf(stdout, "%s\n", __func__);
	dbus_message_iter_append_basic(iter, DBUS_TYPE_INT32, &cmee);
}

static int cmd_manet_setcmee(char *args[], int num, struct connman_option *options)
{
	DBusMessage * msg;
	if (num > 1)
		cmee = strtol(args[1], NULL, 10);
	fprintf(stdout, "%s,args num:%d, cmee=%d\n", __func__, num, cmee);

	msg = dbus_message_new_method_call(CONNMAN_SERVICE, "/manet",
			"net.connman.Manet", "SetCmee");
	if (!msg)
		return -ENOMEM;

	dbus_message_set_auto_start(msg, FALSE);
	dbus_message_append_args(msg, DBUS_TYPE_INT32, &cmee, DBUS_TYPE_INVALID);
	dbus_message_set_no_reply(msg, TRUE);
	g_dbus_send_message(connection, msg);

	return 0;
//	return __connmanctl_dbus_method_call(connection, CONNMAN_SERVICE,
//			"/manet", "net.connman.Manet", "SetCmee",
//			NULL, NULL, append_cmee, NULL);
}

static int manet_cmee_print(DBusMessageIter *iter, const char *error,
		void *user_data)
{
	if (error) {
		fprintf(stderr, "[%s]Error: %s", __func__, error);
		return 0;
	}
...
static int cmd_manet_getcmee(char *args[], int num, struct connman_option *options)
{
	return __connmanctl_dbus_method_call(connection, CONNMAN_SERVICE,
			"/manet", "net.connman.Manet", "GetCmee",
			manet_cmee_print, NULL, NULL, NULL);
}

static int cmd_manet_sendsig(char *args[], int num, struct connman_option *options)
{
	return __connmanctl_dbus_method_call(connection, CONNMAN_SERVICE,
			"/manet", "net.connman.Manet", "SendSig",
			manet_cmee_print, NULL, NULL, NULL);
}

static int manet_grpcfg_print(DBusMessageIter *iter, const char *error,
		void *user_data)
{
	if (error) {
		fprintf(stderr, "[%s]Error: %s", __func__, error);
...
static void append_grpcfg(DBusMessageIter *iter, void *data)
{
	DBusMessageIter array, body, head, entry;
	guint16 v1 = 16;
	guint32 v2 = 32;
	guint8  v3 = 8;
	int i;
	...
	dbus_message_iter_open_container(&array, DBUS_TYPE_STRUCT,
			NULL, &head);
	dbus_message_iter_append_basic(&head, DBUS_TYPE_UINT16, &v1);
	dbus_message_iter_close_container(&array, &head);

	dbus_message_iter_append_basic(&array, DBUS_TYPE_UINT32, &v2);
	v2 = 64;
	dbus_message_iter_append_basic(&array, DBUS_TYPE_UINT32, &v2);

	dbus_message_iter_open_container(&array, DBUS_TYPE_ARRAY,
			DBUS_TYPE_UINT32_AS_STRING, &entry);
	for (i = 0; i < 6; i++) {
		fprintf(stdout, "append byte[%d]:%d\n", i, v2);
		dbus_message_iter_append_basic(&entry, DBUS_TYPE_UINT32, &v2);
		v2++;
	}
...
/*
	dbus_message_iter_open_container(&array, DBUS_TYPE_STRUCT,
			NULL, &entry);
	dbus_message_iter_append_basic(&array, DBUS_TYPE_UINT16, &v1);
	dbus_message_iter_append_basic(iter, DBUS_TYPE_UINT16, &v1);
	dbus_message_iter_append_basic(iter, DBUS_TYPE_UINT16, &v1);
	dbus_message_iter_append_basic(iter, DBUS_TYPE_UINT16, &v1);
	dbus_message_iter_close_container(&array, &entry);

	dbus_message_iter_append_basic(iter, DBUS_TYPE_UINT16, &v1);
	dbus_message_iter_append_basic(iter, DBUS_TYPE_BYTE, &v3);

	dbus_message_iter_open_container(&array, DBUS_TYPE_ARRAY,
			DBUS_TYPE_BYTE_AS_STRING,
			&entry);
	dbus_message_iter_append_basic(&entry, DBUS_TYPE_BYTE, &v3);
	dbus_message_iter_append_basic(&entry, DBUS_TYPE_BYTE, &v3);
	dbus_message_iter_close_container(&array, &entry);
...
static int cmd_manet_setgrpcfg(char *args[], int num, struct connman_option *options)
{
	return __connmanctl_dbus_method_call(connection, CONNMAN_SERVICE,
			"/manet", "net.connman.Manet", "SetGrpCfg",
			manet_grpcfg_print, NULL, append_grpcfg, NULL);
}

static int manet_linksetup_print(DBusMessageIter *iter, const char *error,
		void *user_data)
{
	if (error) {
		fprintf(stderr, "[%s]Error: %s", __func__, error);
...
static void append_linksetup(DBusMessageIter *iter, void *data)
{
	DBusMessageIter array, body, head, entry;
	guint16 v1 = 16;
	guint32 v2 = 32;
	...
	dbus_message_iter_append_basic(&array, DBUS_TYPE_BYTE, &v3);
	v3 = 3;
	dbus_message_iter_append_basic(&array, DBUS_TYPE_BYTE, &v3);
	v3 = 0;
	dbus_message_iter_append_basic(&array, DBUS_TYPE_BYTE, &v3);
	...
	/*ay*/
	dbus_message_iter_open_container(&array, DBUS_TYPE_ARRAY,
			DBUS_TYPE_BYTE_AS_STRING, &head);
	for(i=0; i<64; i++)
	{
		v3 = i +1;
		dbus_message_iter_append_basic(&head, DBUS_TYPE_BYTE, &v3);
	}
	dbus_message_iter_close_container(&array, &head);
	...
	/*aq*/
	dbus_message_iter_open_container(&array, DBUS_TYPE_ARRAY,
			 DBUS_TYPE_UINT16_AS_STRING, &head);
	for(i=0; i<4; i++)
	{
		v1 = i + 1;
		dbus_message_iter_append_basic(&head, DBUS_TYPE_UINT16, &v1);
...
static int cmd_link_setup(char *args[], int num, struct connman_option *options)
{
	return __connmanctl_dbus_method_call(connection, OFONO_SERVICE,
			"/manet", "org.ofono.Manet", "LinkSetup",
			manet_linksetup_print, NULL, append_linksetup, NULL);
}
		...
		DBusMessage *message, void *user_data)
{
	...
	if (!strcmp(interface, "net.connman.Agent") ||
			...
			!strcmp(interface, "net.connman.Manet"))
		return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	...
	if (dbus_message_is_signal(message, "net.connman.Manet",
					"WnuGrpInfoCfgRsp")) {
...
static void monitor_del(char *interface)
{
	...
	fprintf(stdout, ": intf:%s", interface);
...
static struct connman_option monitor_options[] = {
	...
	{"vpnconnection", 'C', "[off]            Monitor only VPN "
	 "connections" },
	{"manet", 'A', "[off]	Mointor only Manet"},
	{ NULL, }
};
...
static const struct {
	...
	{ "manet_rildinit", NULL, NULL,				cmd_manet_rildinit,
		"init manet rild socket interface", NULL},
	{ "manet_setcmee",		"0|1|2",	NULL,				cmd_manet_setcmee,
		"manet set cmee", NULL },
	{ "manet_getcmee",		"0|1|2",	NULL,				cmd_manet_getcmee,
		"manet get cmee", NULL },
	{ "manet_setgrpcfg",		"grpcfg",	NULL,				cmd_manet_setgrpcfg,
		"manet set group config", NULL },
	{ "manet_linksetup",		"linksetup",	NULL,				cmd_link_setup,
		"link setup", NULL },
	{ "manet_sendsig",		NULL,	NULL,				cmd_manet_sendsig,
		"manet send signal", NULL },
