...
public:
    ...
    //ptt
    Q_INVOKABLE unsigned joinGroup(QString groupAddr, unsigned port); // returns groupId
    Q_INVOKABLE int leaveGroup(unsigned groupId);
    Q_INVOKABLE int getPttState(unsigned groupId);    // return type is PttState
    Q_INVOKABLE int setPttState(unsigned groupId, int state); // state is of type PttState
