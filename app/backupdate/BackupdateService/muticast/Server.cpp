...
//pthread_cond_t cond;
#include "lockDefine.h"
Server::Server(int netType,std::string mcAddr,int mcPort,int udpPort,MutiCast* pMutiCast,int groupId)
{
    //m_pMutiCast = pMutiCast;
    m_iGroupId = groupId;
    udpServerInstance = new UdpServer(udpPort,groupId);
    mutiCastServerInstance = new MutiCastServer(mcAddr,mcPort,pMutiCast,groupId);
...
void Server::SendData(int msgtype,int fileid,std::string path)
{
    //InitRemoteIps();
    ...
    int maxseq = m_pSFileData->SplitFile(msgtype,fileid);
...
{
    std::string localIp;
    LocalIaAddress::getInstance()->GetLocalIp(-1,std::string("eth0"),localIp);


    std::vector<std::string> vRemoteInfos;
    ...
    for(int t = 0 ; t < m_vRemoteInfos.size() ; ++t)
    {
        ...
        //if(localIp != m_vRemoteInfos.at(t).c_str());
        if(!LocalIaAddress::getInstance()->IsIpSame(localIp,m_vRemoteInfos.at(t)))
            vRemoteInfos.push_back(m_vRemoteInfos.at(t));
    ...
    {
        std::cout<<"vRemoteInfos size:"<<vRemoteInfos.size()<<std::endl;
        for(int t = 0 ; t < vRemoteInfos.size() ; ++t)
        {
            memcpy(localaddr,vRemoteInfos.at(t).c_str(),IP_LEN);
            //if(localIp != vRemoteInfos.at(t).c_str());
            m_pUnCompleTab->AddTab(i,localaddr,mucastaddr);
        }
    }
    m_pUnCompleTab->SetAllIpNum(m_vRemoteInfos.size());
}
