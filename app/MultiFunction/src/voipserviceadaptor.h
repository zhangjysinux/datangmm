...
public:
    explicit VoipServiceAdaptor(const QString serviceName = "com.sinux.multifunction", QObject *parent = 0);
    Q_INVOKABLE int initialize(unsigned sipPort = 8080, unsigned rtpPort = 33333, unsigned rtpPortRange = 10);
    // returns callId
    Q_INVOKABLE int makeCall(QString ipAddr);
    Q_INVOKABLE int makeVideoCall(QString ipAddress);
