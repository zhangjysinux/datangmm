    ...
    m_interface(serviceName, "/", "com.sinux.DBus.voip", QDBusConnection::sessionBus())
{
    ...
    sessionBus.connect(serviceName, "/", "com.sinux.DBus.voip", "signalV4l2FrameData", this, SLOT(onV4l2FrameData()));
...
int VoipServiceAdaptor::makeVideoCall(QString ipAddress)
{
    int error = -1;
    QDBusReply<int> reply = m_interface.call("onMakeVideoCall", ipAddress);
    if(reply.isValid())
    {
        error = reply.value();
    }
...
bool VoipServiceAdaptor::getIsComming()
{
    int error = -1;
    QDBusReply<bool> reply = m_interface.call("onGetIsComming");
    if(reply.isValid())
    {
        error = reply.value();
    }
    ...
    {
        qDebug() << "onGetIsComming reply error:" << reply.error();
    }
    return error;
}
