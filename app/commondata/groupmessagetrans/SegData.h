...
#define MAX_SEG 60000
#define IP_LEN 15
#define FILE_NAME_SIZE 50
#define ACK_SIZE (8+IP_LEN+IP_LEN)
#define SEG_SIZE (24+IP_LEN+MAX_SEG+FILE_NAME_SIZE)
...
struct SegData
{
    ...
    int Serialization(unsigned char* pBuffer )
    {
        ...
        memcpy(pBuffer+20,&checkSum,4);
        memcpy(pBuffer+24,fileName,FILE_NAME_SIZE);
        memcpy(pBuffer+24+FILE_NAME_SIZE,sourceIP,IP_LEN);
        memcpy(pBuffer+24+FILE_NAME_SIZE+IP_LEN,buffer,MAX_SEG);
        return (24+FILE_NAME_SIZE+IP_LEN+MAX_SEG);
    }
