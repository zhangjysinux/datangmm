...
#else
    #define DIAL_HANDLER_SERVICE QLatin1String("com.dtt.CommTransService")
    #define DIAL_HANDLER_INTERFACE QLatin1String("com.dtt.CommTransService.DialHandler")
#endif
...
{
    CTS_PRINT << endl;
    qDebug() << "call dialNumber" << m_handlerId;

    //"/handle"+
//    if (SWITCH == "sinux")
        m_interface = new QDBusInterface(DIAL_HANDLER_SERVICE, "/handle"+m_handlerId, DIAL_HANDLER_INTERFACE, QDBusConnection::sessionBus());
//    else
//        m_interface = new QDBusInterface(DIAL_HANDLER_SERVICE, m_handlerId, DIAL_HANDLER_INTERFACE, QDBusConnection::sessionBus());
    ...
    else
    {
        ...
        //"/handle"+
//        if (SWITCH == "sinux"){
            sessionBus.connect(DIAL_HANDLER_SERVICE, "/handle"+m_handlerId, DIAL_HANDLER_INTERFACE, "statusChanged", this, SLOT(onStatusChanged(QString,int)));
            sessionBus.connect(DIAL_HANDLER_SERVICE, "/handle"+m_handlerId, DIAL_HANDLER_INTERFACE, "error", this, SLOT(onError(QString)));
            sessionBus.connect(DIAL_HANDLER_SERVICE, "/handle"+m_handlerId, DIAL_HANDLER_INTERFACE, "multipartyChanged", this, SLOT(onMultipartyChanged()));
//        }
//        else {
...
void DialHandler::onStatusChanged(QString handlerId, int state)
{
    CTS_PRINT << endl;
