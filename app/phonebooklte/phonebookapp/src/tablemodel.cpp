...
QVariant ContacterModel::data(const QModelIndex &index, int role) const
{
    ...
    else
    {
        ...
        Contacter contacter = m_contacterAdaptor->getContacter(rowId);
        if (columnIdx == 0) {
            ...
            if (contacter.namePinyin >= "0" && contacter.namePinyin <= "9") {
            ...
            value = QVariant::fromValue(contacter.selectedStatus);
        }
    }
    return value;
}
...
void ContacterModel::setModelList(const int &netType, const int &contacterType, const QString& searchText, QStringList memberList)
{
    beginResetModel();
    if (netType != 0) {
        if (searchText == "") {
...
void ContacterModel::setModelList(const int &netType, const int &contacterType, const QString &searchText)
{
    beginResetModel();
    m_idList  = m_contacterAdaptor->searchContactorsExcept(contacterType, netType, searchText, QStringList());
    qDebug()<<"setModelList"<<m_idList.count();
    endResetModel();
}
...
QString ContacterModel::getContacterName(QString contacterId)
{
    Contacter contacter;
...
/*info:id*/
int ContacterModel::getCurrentIndex(QString info)
{
    int temp = 0;
