...
//        account = "100110004";
    ...
    lteInstance->login(localContacter.pocId, "123456", LTE_NETWORK, "170.10.1.111", 4001);
...
void GroupInterface::removeGroupNetwork(const Group &group)
{
    if(group.networkType == LTE_NETWORK)
    {
        ...
        QStringList cIdList;
        cIdList << group.creator  << group.leaders << "237"; //QStringList("236")//<< groupData.creator
        qDebug() << "addGroupNetwork: " << cIdList;
    //    group.
        QList<QT_DTT_JJF::UserInfo> userInfoList;
        for(int i=0; i<cIdList.count(); i++)
        {
            ...
            QT_DTT_JJF::UserInfo userInfo;
            userInfo.userId = userInfoList.size()+1;
            userInfo.network = group.networkType;
            userInfo.addr = getPhoneNumberType(contacter.numbers, group.networkType).number;
            userInfo.userName = contacter.surname+contacter.name;
            userInfo.lte_ip = getPhoneNumberType(contacter.numbers, group.networkType).number;

            QString ipEnd = userInfo.lte_ip.section(".", -1);
            if(ipEnd.toInt() == 236)
            {
                userInfo.lte_pocId = QString::number(100110002);
            }
            else if(ipEnd.toInt() == 237)
            {
                userInfo.lte_pocId = QString::number(100110003);
            }
            else if(ipEnd.toInt() == 238)
            {
                userInfo.lte_pocId = QString::number(100110004);
            }
