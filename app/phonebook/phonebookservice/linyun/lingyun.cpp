...
#include <QDebug>
#include <QFile>
#define DATA_PATH "/data/apps/com.sinux.phonebook
#define APP_PATH "/data/data/com.sinux.phonebook
...
int LingYun::init()
{
    ...
    QString init_config = "";
    init_config += "appKey=c85d54f1";              //灵云应用序号
    init_config += ",developerKey=712ddd892cf9163e6383aa169e0454e3"; //灵云开发者密钥
    init_config += ",cloudUrl=http://test.api.hcicloud.com:8888";         //灵云云服务的接口地址
    init_config += ",capKey=tts.local.synth";
    init_config += QString(",authpath=%1").arg("/data/data/com.sinux.phonebook");         //授权文件所在路径，保证可写
    init_config += QString(",logfilepath=%1").arg("/data/data/com.sinux.phonebook");   //日志的路径
    ...
    initAudioOutput();
    //transToTts("adfad");
//    m_ttsSynth->FunctionFunc("tts.local.synth","/data/apps/com.sinux.phonebook/libs","/home/user/tts");
...
void LingYun::writeToFile(QString msg)
{
    ...
    file.close();
    m_enable = false;
    qDebug() << "Zhangjy LingYun FunctionFunc";
    m_ttsSynth->FunctionFunc("tts.local.synth","/data/apps/com.sinux.phonebook/libs","/home/user");
    play();
