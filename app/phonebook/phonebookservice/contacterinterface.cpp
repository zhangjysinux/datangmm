...
bool ContacterInterface::onUpdateContacter(const Contacter &contacter)
{
    ...
    if(lastErrorIsNoError())
    {
        ...
        //update local contacter
        QString phoneNumberWifi = getPhoneNumberType(updatedContacter.numbers, WIFI_NETWORK).number;
        QString phoneNumberAdHoc = getPhoneNumberType(updatedContacter.numbers, AD_HOC_NETWORK).number;
        QString phoneNumberLte = getPhoneNumberType(updatedContacter.numbers, LTE_NETWORK).number;

        QStringList ips = ConnectionDatabase::getIp();
        if( ips.contains(phoneNumberWifi) ||
            ips.contains(phoneNumberAdHoc) ||
            ips.contains(phoneNumberLte) )
        {
            addUpdateLocalInfo(updatedContacter);
        }
...
{
    qDebug() << "xxxxxxxx" << contacter.pocId << contacter.snNumber;
    ...
    if(contacter.name.isEmpty())
        return false;
    setLocalInfo(contacter);
    return true;
    int row = 0;
    QModelIndex idIndex = m_localModel.index(row, 0);
    m_localModel.setData(idIndex, contacter.id);

    QModelIndex surnameIndex = m_localModel.index(row, 1);
    m_localModel.setData(surnameIndex, contacter.surname);

    QModelIndex nameIndex = m_localModel.index(row, 2);
    m_localModel.setData(nameIndex, contacter.name);

    QModelIndex titleIndex = m_localModel.index(row, 3);
    m_localModel.setData(titleIndex, contacter.title);

    QModelIndex numbersIndex = m_localModel.index(row, 4);
    ...
    QModelIndex photoIndex = m_localModel.index(row, 5);
    m_localModel.setData(photoIndex, "file://"+newAddress);

    QModelIndex leadersIndex = m_localModel.index(row, 6);
    QString leaders = convertLeadersToString(contacter.leaders);
    m_localModel.setData(leadersIndex, leaders);

    QModelIndex snNumberIndex = m_localModel.index(row, 7);
    m_localModel.setData(snNumberIndex, contacter.snNumber);

    QModelIndex bluetoothIndex = m_localModel.index(row, 8);
    m_localModel.setData(bluetoothIndex, contacter.bluetooth);
    ...
    QModelIndex namePinyinIndex = m_localModel.index(row, 9);
    m_localModel.setData(namePinyinIndex, namePinyin);

    QModelIndex typeIndex = m_localModel.index(row, 10);
    m_localModel.setData(typeIndex, contacter.type);

    QModelIndex selectedStatusIndex = m_localModel.index(row, 11);
    m_localModel.setData(selectedStatusIndex, contacter.selectedStatus);

    QModelIndex pocidIndex = m_localModel.index(row, 12);
    m_localModel.setData(pocidIndex, contacter.pocId);
...
void ContacterInterface::addUpdateLocalInfo(Contacter data)
{
    m_localModel.select();
    int rowCount = m_localModel.rowCount();
    if(rowCount <= 0)
    {
        m_localModel.insertRow(rowCount);
    }
