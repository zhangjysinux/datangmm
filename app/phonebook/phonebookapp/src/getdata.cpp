    ...
    QObject(parent), m_sessionBus("")
{
    ...
    connect(m_messageAdaptor, SIGNAL(signalP2PConnected(QString,int)),
...
bool GetData::getGroups(bool bIsDelete)
{
    ...
    for(int i = 0, iend = groupList.count(); i < iend; i++) {
        ...
        qDebug() << "group members:" << group.members.count() << group.members;
...
void GetData::getContacter(QString id)
{
    ...
    emit signalContacterInfo("POC",contacter.pocId);
    if (contacter.type == 1) {
        emit signalContacterInfo("设备序列号",contacter.snNumber);
    }
}

void GetData::updateLocalInfo(QString name, QString title, QString snNum, QString bluetooth, QString pocid)
{
...
void GetData::updateContacter(QString name, QString title, QString num, QString photopath ,QString pocid)
{
...
//        return;
    ...
    if(messageType == 1)//text for adhoc for gxdatabreak
    {
        if(value.contains("GXData:"))
        {
            //for gxdata adhoc return
            return;
        }
    }
...
void GetData::onSetPropertyType(int type)
{
    if (type == 1)
        onSetPropertyValue(1);
    else if (type == 0)
        onSetPropertyValue(0);
...
{
    qDebug() << "GetData::onSignalP2PConnected: " << groupId << state;
    if(m_groupIdStatusMap.contains(groupId))
        m_groupIdStatusMap.remove(groupId);

    m_groupIdStatusMap.insert(groupId, state);
    emit signalP2PConnected(groupId, state);
}

int GetData::onGetP2PConnectedStatus(const QString &groupId)
{
    return m_groupIdStatusMap.value(groupId, 0);
}
