...
{
    qDebug() << "xxxxxxxx" << localInfo.pocId << localInfo.snNumber;
...
void ContacterInterfaceAdaptor::setAllSelectedStatus(const int &status)
{
    QDBusReply<void> contactersReply = m_interface.call("onSetAllSelectedStatus", status);
    if(contactersReply.isValid())
        qDebug() << "onSearchContacterBluetooth reply success";
    else
        qDebug() << "onSearchContacterBluetooth reply error:" << contactersReply.error();
...
bool ContacterInterfaceAdaptor::canEditContacter(const QString &id)
{
    return true;
    QDBusReply<bool> contactersReply = m_interface.call("onCanEditContacter", id);
    if(contactersReply.isValid())
        qDebug() << "onCanEditContacter reply success";
    else
        qDebug() << "onCanEditContacter reply error:" << contactersReply.error();

    return contactersReply.value();
}
