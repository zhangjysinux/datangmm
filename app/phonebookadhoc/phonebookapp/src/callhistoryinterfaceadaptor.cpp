...
QStringList CallHistoryInterfaceAdaptor::getCallHistoryList(const QDateTime &startTime, const QDateTime &endTime)
{
    qDebug()<<"onGetCallHistoryList startTime endTime in app";
    QStringList values;
    QDBusReply<QStringList> historyReply = m_interface.call("onGetCallHistoryList", startTime, endTime);
    if(historyReply.isValid())
        values = historyReply.value();
    else
        qDebug() << "onGetCallHistoryList startTime endTime reply error:" << historyReply.error();

    return values;
}

QStringList CallHistoryInterfaceAdaptor::getCallHistoryList(const int &startIndex, const int &endIndex)
{
    qDebug()<<"onGetCallHistoryList startIndex endIndex in app";
    QStringList values;
    QDBusReply<QStringList> historyReply = m_interface.call("onGetCallHistoryList", startIndex, endIndex);
    if(historyReply.isValid())
        values = historyReply.value();
    else
        qDebug() << "onGetCallHistoryList startIndex endIndex reply error:" << historyReply.error();
...
QStringList CallHistoryInterfaceAdaptor::getCallHistoryByNumber(const QString &number)
{
    qDebug()<<"getCallHistoryByNumber in app";
    QStringList values;
    QDBusReply<QStringList> historyReply = m_interface.call("onGetCallHistoryByNumber",number);
    if(historyReply.isValid())
        values = historyReply.value();
    else
        qDebug() << "onGetCallHistoryByNumber reply error:" << historyReply.error();
...
QStringList CallHistoryInterfaceAdaptor::searchCallHistory(const QString &searchText, const int &type)
{
    QStringList values;
    QDBusReply<QStringList> historyReply = m_interface.call("onSearchCallHistory",searchText, type);
    if(historyReply.isValid())
        values = historyReply.value();
    else
        qDebug() << "onSearchCallHistory reply error:" << historyReply.error();
