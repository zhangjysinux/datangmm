...
MyApp_Workspace::MyApp_Workspace()
	: CWorkspace(), accountId_(-1), currentCall_(-1),
	preview_on(false), video_(NULL), video_prev_(NULL)
{
    ...
    m_view = new QWidget;
    initLayout();
    QObject::connect(m_view, SIGNAL(quit()), qApp, SLOT(quit()));
...
void MyApp_Workspace::initLayout()
{
	...
	hbox_url->addWidget(url_ = new QLineEdit(tr("sip:vidgui@192.168.3.167:5080")), 1);
...
void MyApp_Workspace::showStatus(const char *msg)
{
...
void MyApp_Workspace::doShowStatus(const QString& msg)
{
...
void MyApp_Workspace::showError(const char *title, pj_status_t status)
{
...
void MyApp_Workspace::onVidEnabledChanged(int state)
{
...
void MyApp_Workspace::onNewCall(int cid, bool incoming)
{
	pjsua_call_info ci;
...
void MyApp_Workspace::onCallReleased()
{
...
void MyApp_Workspace::call()
{
	if (callButton_->text() == "Answer") {
		...
		pjsua_call_setting_default(&call_setting);
		//    call_setting.flag=PJSUA_CALL_UPDATE_CONTACT;
		//    call_setting.req_keyframe_method=PJSUA_VID_REQ_KEYFRAME_RTCP_PLI;
        ...
        pj_ansi_strncpy(uri, dst.toLocal8Bit().data(), sizeof(uri));
...
void MyApp_Workspace::initVideoWindow()
{
	pjsua_call_info ci;
	unsigned i;
...
void MyApp_Workspace::on_reg_state(pjsua_acc_id acc_id)
{
	pjsua_acc_info info;
...
void MyApp_Workspace::on_call_state(pjsua_call_id call_id, pjsip_event *e)
{
	pjsua_call_info ci;
...
void MyApp_Workspace::on_incoming_call(pjsua_acc_id acc_id, pjsua_call_id call_id,
	pjsip_rx_data *rdata)
{
...
void MyApp_Workspace::on_video_stream_data(int* acc_id)
{
    int c = 0;
    int kk = 0;
}
void MyApp_Workspace::on_call_media_state(pjsua_call_id call_id)
{
    return;
	pjsua_call_info ci;
...
{
	pInstance->on_reg_state(acc_id);
}

static void on_call_state(pjsua_call_id call_id, pjsip_event *e)
{
	pInstance->on_call_state(call_id, e);
}

static void on_incoming_call(pjsua_acc_id acc_id, pjsua_call_id call_id,
	pjsip_rx_data *rdata)
{
	pInstance->on_incoming_call(acc_id, call_id, rdata);
}

static void on_call_media_state(pjsua_call_id call_id)
{
	pInstance->on_call_media_state(call_id);
}
static void on_video_stream_data(int* acc_id)
{
    pInstance->on_video_stream_data(acc_id);
}
//
// initStack()
//
bool MyApp_Workspace::initStack()
{
	pj_status_t status;
    ...
    video_config vi_cfg;
    vi_cfg.on_video_stream_data=&::on_video_stream_data;
    pjvideo_init(&vi_cfg);
    ...
    acc_cfg.vid_in_auto_show = PJ_TRUE;//add by TYZ
#if USE_REGISTRATION
