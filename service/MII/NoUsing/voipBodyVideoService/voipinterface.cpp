...
int VoipInterface::heartBagFlag = -1 ;//chu shi zhi
...
int VoipInterface::onMakeVideoCall(QString ipAddress)
{
    ...
    onHangup(0);
    heartBagFlag = 1 ;
    m_dbusinterface->call("onStopServiceTimer");
    //end by michael zheng
    m_bIsStart = true;
    qDebug() << "onMakeVideoCall ipAddress=" << ipAddress << endl;
    ...
    qDebug()<<"-----start heart----1---";
//    QTest::qSleep(500);

    //by michael zheng 2017.4.5 for using heart bag
    m_dbusinterface->call("onStartLinkSignal",ipAddress);
    qDebug()<<"-----start heart----4---";
...
int VoipInterface::onAnswer(int callId)
{
    ...
    //by michael zheng 2017.4.5 for using heart bag
    m_dbusinterface->call("onStartLinkSignal","192.168.43.4");
    qDebug()<<"-----start heart----4---";
...
// by michael zheng 2017.4.5
void VoipInterface::onAgainLink(QString ipAddr, int state)
{
    qDebug()<<"---------onAgainLink-----------";
    onMakeVideoCall(ipAddr);
}
void VoipInterface::onRecvData(QString recvipdata, int state)
{
    qDebug()<<"---------onRecvData-----------";
    m_dbusinterface->call("onRecvHeartData",recvipdata,heartBagFlag);
}
//end by michael zheng
...
void VoipInterface::heartLinkInit()
{
    qDebug()<<"---------heartLinkInit-------";

    // by michael zheng 2017.4.5 for using heart bag
    m_dbusinterface = new QDBusInterface("com.sinux.myListenHeart","/myHeart","com.sinux.DBus.MyHeartBag");
    m_dbusConnection.connect("com.sinux.myListenHeart",
                             "/myHeart",
                             "com.sinux.DBus.MyHeartBag",
                             "signalSendHeartData",
                             this,
                             SLOT(onRecvData(QString,int)));
    qDebug()<<"-----start heart----2---";
...
//                             "signalAgainLink",
//                             this,
//                             SLOT(onAgainLink(QString,int)));
    qDebug()<<"-----start heart----3---";
