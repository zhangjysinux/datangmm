...
signals:
    ...
    //by michael zheng 2017.4.5 for heartbag
    void signalSendHeartData(QString ipdata,int state);
private slots:
    void onAgainLink(QString ipAddr,int state);
    void onRecvData(QString recvipdata,int state);
    // end by michael zheng
...
private:
    ...
    QDBusInterface *m_dbusinterface;//by michael for heartbag
    ...
    QMutex m_mutex ;
    static int heartBagFlag;    //by michael for heartbag
};
