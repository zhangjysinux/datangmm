...
Endpoint *ep;
AudioMediaPlayer *player; //using by michael zheng 2017.4.18
AudioMediaPlayer *outgoingPlayer;   //using by michael zheng 2017.4.18
...
static void putV4l2Frame(unsigned char *buf,int width,int height,int pixfmt)
{
    ...
    qDebug()<<"putV4l2Frame"<<__FILE__<<__LINE__;
    if(shmid == -1)
    {
    ...
    pthread_cond_signal(&cond); //using by michael zheng 2017.4.18
    frametype = local;  //using by michael zheng 2017.4.18
...
static void putFrame(unsigned char *buf,int *bSize)
{
    ...
    qDebug()<<"putFrame"<<__FILE__<<__LINE__;
   ...
   // printf("width:%d\n",width);
    ...
    pthread_cond_signal(&cond); //using by michael zheng 2017.4.18
    frametype = remote; //using by michael zheng 2017.4.18
    qDebug()<<"putFrame"<<__FILE__<<__LINE__;
    pthread_mutex_unlock(&m_lock);
}
...
//    ep_cfg.medConfig.quality = 4; //nousing by michael zheng 2017.4.18
...
#ifdef voipAdHocService
//    ep->codecSetPriority("speex/8000/1", 200);    //no using by michael zheng 2017.4.18
    qDebug() << "ep->codecSetPriority(\"speex/8000/1\", 200)";
#else
    ep->codecSetPriority("iLBC/8000/1", 200);
#endif

    //start no using by michael zheng 2017.4.18
...
//                 << endl;
//    }
    //end no using by michael zheng 2017.4.18
    ...
    player = new AudioMediaPlayer;  //using by michael zheng 2017.4.18
    player->createPlayer("/usr/share/voipService/清晨乐章2.wav");   //using by michael zheng 2017.4.18

    outgoingPlayer = new AudioMediaPlayer;  //using by michael zheng 2017.4.18
    outgoingPlayer->createPlayer("/usr/share/voipService/朝气蓬勃2.wav");// using by michael zheng 2017.4.18
    ...
    for(int i = 0; i < count;++i)
    {
        ...
        //------------------------------ 提高画质
        param.dec_fmt.det.vid.fps.num = 20; //设置帧率
        param.enc_fmt.det.vid.fps.num = 20;
        param.dec_fmt.det.vid.avg_bps = 512;//设置比特率
        param.dec_fmt.det.vid.max_bps = 512;
        param.enc_fmt.det.vid.avg_bps = 512;
        param.enc_fmt.det.vid.max_bps = 512;
...
//    system("cd /home/user/ && mkdir voip____");   // no using by michael zheng 2017.4.18
    ...
    VoipVideoServer *server = new VoipVideoServer();    //using by michael zheng 2017.4.18
    server->start();    //using by michael zheng 2017.4.18
...
//    qDebug() << "pppppppppppppppppp" << sdlvideoacc.SDL_Callback<<endl;
    ...
    //  7010,6010,14
//    service->onInitialize(5360, 5361, 20);
    // end by michael zheng
    ...
    // by michael zheng 2017.4.6
    service->heartLinkInit();
    // end by michel zheng
    int ret = a.exec();
...
//    }
    ...
    static QMutex mutex;
    QMutexLocker locker(&mutex);    //using by michael zheng 2017.4.18
//    mutex.lock(); // no using by michael zheng 2017.4.18
    QString logFileName;
#ifdef voipAdHocService
    logFileName = "adhoc";
#elif voipLteService
...
//    std::cout << msg.toLocal8Bit().data() << std::endl;
    file.flush();
//    mutex.unlock();   // no using by michael zheng 2017.4.18
}
