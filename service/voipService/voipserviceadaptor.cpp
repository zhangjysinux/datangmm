    ...
    m_interface(serviceName, "/", "com.sinux.DBus.voip", QDBusConnection::sessionBus())
{
    ...
    sessionBus.connect(serviceName, "/", "com.sinux.DBus.voip", "signalCallError", this, SIGNAL(signalCallError(int)));
...
{
    qDebug()<<"onVidInputChanged in VoipServiceAdaptor" << callId;
    int error = -1;
...
{
    qDebug() << "GetAudOrVideo in voipserviceadaptor";
    int error = -1;
...
int VoipServiceAdaptor::getCallError(int callId)
{
    qDebug() << "getCallError in voipserviceadaptor";
    int error = -1;
    QDBusReply<int> reply = m_interface.call("onGetCallError",callId);
    if(reply.isValid())
    {
        error = reply.value();
    }
    ...
    {
        qDebug() << "onGetCallError reply error:" << reply.error();
    }
    return error;
}
...
{
    qDebug() << "onSignalPttRecordFileFinished" << groupId << fileName;
