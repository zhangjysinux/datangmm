...
pj_status_t PttMediaStream::createTransport()
{
    ...
    close(sockfd);
    qDebug() << "socket lmi45 find: " << status;

    if(status != PJ_SUCCESS)
    {
        sockfd = socket(AF_INET, SOCK_DGRAM, 0);
        strcpy(ifr.ifr_name, "adhoc0");
        status = ioctl(sockfd, SIOCGIFADDR, &ifr);
        close(sockfd);
        qDebug() << "socket adhoc0 find: " << status;
    }
    ...
    if(status != PJ_SUCCESS)
    {
        ...
        status = ioctl(sockfd, SIOCGIFADDR, &ifr);
        close(sockfd);
        qDebug() << "socket wlan0 find: " << status;
    }
    ...
    qDebug() << "addr: " << addr->sin_port;

    pj_bzero(&si, sizeof(pjmedia_sock_info));
    si.rtp_sock = si.rtcp_sock = PJ_INVALID_SOCKET;

    status = pj_sock_socket(af, pj_SOCK_DGRAM(), 0, &si.rtp_sock);
    qDebug() << "pj_sock_socket: " << status << si.rtcp_sock;
    if (status != PJ_SUCCESS)
        goto on_error;

    status = pj_sockaddr_init(af, &si.rtp_addr_name, NULL, m_port);
    qDebug() << "pj_sockaddr_init: " << status;
    if (status != PJ_SUCCESS)
        goto on_error;

    //reuse addr
    status = pj_sock_setsockopt(si.rtp_sock, pj_SOL_IP(), pj_SO_REUSEADDR(),
                                &enabled, sizeof(enabled));
    qDebug() << "pj_sock_setsockopt pj_SO_REUSEADDR: " << status;
    if (status != PJ_SUCCESS)
        goto on_error;
...
bool PttMediaManager::init()
{
    ...
    codec_config.speex.quality = 4;
    status = pjmedia_codec_register_audio_codecs(m_medEndpt, &codec_config);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, false);

    codec_mgr = pjmedia_endpt_get_codec_mgr(m_medEndpt);
    codec_id = pj_str((char *)"speex/8000/1");//iLBC
    status = pjmedia_codec_mgr_find_codecs_by_id(codec_mgr, &codec_id, &count,
                                                 &m_codecInfo, NULL);
    qDebug() << "m_codecInfo"
             << m_codecInfo->encoding_name.ptr
             << m_codecInfo->clock_rate;
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, false);

    status = pjmedia_codec_mgr_set_codec_priority(codec_mgr, &codec_id, 200);
    qDebug() << "pjmedia_codec_mgr_set_codec_priority status: " << status;
